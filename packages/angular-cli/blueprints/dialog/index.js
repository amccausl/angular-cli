...
module.exports = {
  ...
  files: function() {
    ...
    if (this.options && !this.options.spec) {
      fileList = fileList.filter(p => p.indexOf('__name__.dialog.spec.ts') < 0);
    }
  ...
  afterInstall: function(options) {
    ...
    const returns = [];
    const className = stringUtils.classify(`${options.entity.name}Dialog`);
    const fileName = stringUtils.dasherize(`${options.entity.name}.dialog`);
